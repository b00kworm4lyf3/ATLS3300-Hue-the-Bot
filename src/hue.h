...
class Hue{
    public:
        ...
        bool begin();
        void read(); //sample sensor and update current RGB/hex, will also have accel at some point (might want to break into 2 methods)
        void show(); //push current colour (rn to esp32 neopix, will be to screen and 2 external neopix)
        void printRead(); //print to serial

        //current colour -- updated with read()
        uint8_t r, g, b; //0-255
